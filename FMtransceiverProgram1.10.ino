/* V1.10更新内容：
   0. 增加设置界面，用户可以在设置界面自由设置电台的次级参数（累死我了）。
   1. 1.4版本的静噪功能强势回归：当RSSI值低于设定阈值时自动静音（仅在接收模式下生效）。
   2. 设置界面中增加静噪开关（SQL Enable）和静噪阈值（SQL Thresh）的调节。
   3. 主界面RSSI值旁增加“SQL”状态显示（静噪开启且静音时显示，同V1.4）。
   4. 设置界面中增加发射功率（TX Pwr）和音量（Vol）的调节。
   5. 所有设置参数在退出设置时自动存入EEPROM，避免掉电清零。
   更新日期：2026年3月7日
*/
...
void PTTcontrol(void){
  if (buttonState != lastButtonState) { // 检测PTT状态变化（下降沿或上升沿）
    if (buttonState == LOW) {
      TX_Fm(Freq);   // 按下PTT，切换到发射模式
    } else {
      RX_Fm(Freq);   // 松开PTT，切换回接收模式
    }
...
void displayRSSIValue(void){
    ...
    if (squelchEnabled && (RSSI < squelchThreshold)) {
        display.print(" SQL");
    }
} //显示RSSI值及静噪状态
