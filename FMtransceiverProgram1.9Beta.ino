/*1.9Beta测试版
注意：该版本为手动更改频率上限强制超频至航空频率，最大上限127.1MHz
该版本因强行超频，可能会导致工作不稳定，频繁重启，已将发射功率降为最低
本版本仅测试！仅测试！仅测试！禁止在未经允许下擅自烧录使用
更新说明：频率超过108MHz时发射将有概率死机重启，频率超119.7MHz后疑似无法有效接收
*/
...
void handleFreqChange(void){
   ...
   //频率加按键按下时
   if (UpButtonState == LOW && Freq < 12710) {
