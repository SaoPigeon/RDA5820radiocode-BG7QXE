/*1.9R纯接收机版本
更新内容：
1.去掉所有发射功能，只保留接收，硬件功率限制直接为0
2.电台模块超频至127M，覆盖航空段
3.UI显示上升级，航空段显示飞机符号和AIR标识
4.PPT按钮自动搜台，按下即可开启自动搜台功能，自动搜索频率
*/
...
#include <avr/pgmspace.h>
 ...
 const uint8_t PROGMEM sou[] = {0x10,0xA0,0x11,0x2C,0x11,0x24,0x11,0x24,0xFD,0xAC,0x11,0x24,0x11,0x24,0x15,0xFC,0x18,0x20,0x31,0xFC,0xD0,0x88,0x10,0x50,0x10,0x20,0x10,0x50,0x50,0x88,0x23,0x06};
 const uint8_t PROGMEM suo[] = {0x01,0x00,0x01,0x00,0x3F,0xF8,0x01,0x00,0x01,0x00,0x7F,0xFE,0x42,0x02,0x84,0x24,0x1F,0xC0,0x01,0x80,0x06,0x10,0x3F,0xF8,0x01,0x08,0x11,0x20,0x25,0x10,0x42,0x08};
 const uint8_t PROGMEM zhong[] = {0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x3F,0xF8,0x21,0x08,0x21,0x08,0x21,0x08,0x21,0x08,0x21,0x08,0x3F,0xF8,0x21,0x08,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00};
 ...
 const uint8_t PROGMEM magnifier[] = {0x00,0xF0,0x03,0xFC,0x07,0x0E,0x06,0x06,0x0C,0x03,0x0C,0x03,0x0C,0x03,0x0C,0x03,0x06,0x06,0x07,0x0E,0x0F,0xFC,0x1C,0xF0,0x38,0x00,0x70,0x00,0xE0,0x00,0xC0,0x00};
 const uint8_t PROGMEM aircraft[] = {0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x80,0x07,0xE0,0x3F,0xFC,0xFF,0xFF,0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x80,0x03,0xC0,0x07,0xE0,0x1E,0x78};
 const uint8_t PROGMEM jie[] = {0x10,0x80,0x10,0x40,0x13,0xFC,0x10,0x00,0xFD,0x08,0x10,0x90,0x17,0xFE,0x10,0x40,0x18,0x40,0x37,0xFE,0xD0,0x88,0x11,0x08,0x10,0x90,0x10,0x60,0x51,0x98,0x26,0x04};
 const uint8_t PROGMEM shou[] = {0x08,0x40,0x08,0x40,0x48,0x40,0x48,0x80,0x48,0xFE,0x49,0x08,0x4A,0x88,0x48,0x88,0x48,0x88,0x58,0x50,0x68,0x50,0x48,0x20,0x08,0x50,0x08,0x88,0x09,0x04,0x0A,0x02};
...
void handleFreqChange(void){
   ...
   //频率加按键按下时
   if (UpButtonState == LOW && Freq < 12700) {
...
void PTTcontrol(void){
  if (buttonState!= lastButtonState) {// 如果按钮状态发生变化
    if (buttonState == LOW) {
       ...
       display.drawBitmap(56, 10, magnifier, 16, 16, WHITE);
       display.drawBitmap(40, 30, sou, 16, 16, WHITE);
       display.drawBitmap(56, 30, suo, 16, 16, WHITE);
       display.drawBitmap(72, 30, zhong, 16, 16, WHITE);
       display.display();

      wdt_disable();//关狗
      delay(10);
      RdaSeek();
      int temp = Rda5820FreqGet();
    if(temp >= 7600 && temp <= 10800)//校验读取到的频率合法性
      {
      Freq = temp;
      }else{
      return;
    ...
    wdt_enable(WDTO_2S);//重新开狗
    lastButtonState = buttonState;//重新复位last防止重复执行
  }
}//原PTT按钮控制发射状态，现改为启动自动搜台
...
void displayTopTitle(void){
  if (Freq > 10800){
    display.drawBitmap(0, 0, aircraft, 16, 16, WHITE);
  ...
  //上标(测试版使用VFO“频率振荡器”，正式版则用CLG标)
    ...
    display.setTextSize(2);
    display.print("VHF");  
    display.drawBitmap(50, 0, jie, 16, 16, WHITE);
    display.drawBitmap(67, 0, shou, 16, 16, WHITE);
  //显示“VHF接收”
}//顶部标题显示
